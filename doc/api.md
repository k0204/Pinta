...
- Form fields:
  ...
  - `size`: concrete output size, for example `1024x1024`; Nano Banana requests send the configured aspect ratio here and send the `1K`, `2K`, or `4K` tier in `resolution`; the server maps or validates it according to the provider's `imageApi`
...
- Nano Banana requests read the official Google Gemini dimensions from `config/NanoBanana.json`, display its configured `1K` through `4K` resolutions, and use the closest configured size that can contain the source image. Nano Banana ignores provider-catalog size overrides so the client and split preview use this single resolution map. Pinta sends the selected Nano Banana channel ID (`tokenx24` or `visionary`) as the `provider` field.
...
- Agnes, GPT Image, and Nano Banana call `/api/images` with repeated `reference_files`, `prompt`, `size`, and `provider`; Nano Banana also sends the selected `resolution` tier, while the client selects the aspect ratio and tier according to its configured map.

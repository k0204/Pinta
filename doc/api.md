...
- Nano Banana requests read `config/NanoBanana.json`, display its configured `1K` through `4K` resolutions, and use the closest configured size that can contain the source image. Nano Banana ignores provider-catalog size overrides so the client and split preview use this single resolution map. Pinta sends the selected Nano Banana channel ID (`tokenx24` or `visionary`) as the `provider` field.

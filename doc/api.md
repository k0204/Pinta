...
- One request generates one action with a sequential frame grid. Repeating the workflow creates another action attempt under `single-direction-animation/actions/<action>/attempt-NN/source-sequence` and adds a `SingleDirectionAnimationLayer` output.

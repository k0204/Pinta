...
public sealed partial class ToolBoxWidget
{
	...
	private void HandleToolRemoved (BaseTool tool)
	{
        ...
        private static string GetToolGroupKey (BaseTool tool)
        {
                if (tool.ShortcutKey == Gdk.Key.Invalid)
                        return $"tool:{tool.GetType ().Name}";

                return $"shortcut:{tool.ShortcutKey.ToUpper ().Name ()}";
        }

        private static string BuildToolTooltip (BaseTool tool, IReadOnlyCollection<BaseTool> groupedTools)
        {
                List<string> lines = [tool.Name];

                if (tool.ShortcutKey != Gdk.Key.Invalid) {
                        string shortcutLabel = Translations.GetString ("Shortcut key");
                        lines.Add ($"{shortcutLabel}: {tool.ShortcutKey.ToUpper ().Name ()}");
                }
        ...
        private sealed class ToolGroupWidget
        {
                ...
                private void UpdateDisplay ()
                {
                        ...
                        Button.TooltipText = BuildToolTooltip (current_tool, grouped_tools);
                }
        }
}
